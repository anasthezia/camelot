...
$document.ready(function(){
    ...
    if(dim.length){
        switch(dim[0]){
            case "px":return parseFloat(val);
            case "vh":return $(window).height()*(parseFloat(val)/100);
...
function toggleSwiperInnerVideos(swiper){
    var prevSlide=$(swiper.slides[swiper.previousIndex]),nextSlide=$(swiper.slides[swiper.activeIndex]),videos;prevSlide.find("video").each(function(){this.pause();});videos=nextSlide.find("video");if(videos.length){videos.get(0).play();}}
...
if(plugins.counter.length){
    ...
    for(i=0;i<plugins.counter.length;i++){
        ...
        var $this=plugins.counter[i];
        $document.on("scroll",$.proxy(function(){
            if((!$($this).hasClass("animated"))&&(isScrolledIntoView($($this)))){
                console.log('1111111111', $($this)[0].innerText)
                $($this)[0].innerText.countTo(
